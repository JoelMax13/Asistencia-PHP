<?php
    ...
    function TablaObservaciones($conexion, $PRO_ID,$diaActual,$mesActual,$anioActual){
        ...
        while($conjuntoDatos = mysqli_fetch_assoc($numUsu)){
            echo "<tr>".
            MostrarTotal($conexion, $conjuntoDatos['USPR_ID'], $diaActual, $mesActual, $anioActual).
            MostrarObservaciones($conexion, $conjuntoDatos['USPR_ID'],1, $diaActual, $mesActual, $anioActual).
            MostrarObservaciones($conexion, $conjuntoDatos['USPR_ID'],2, $diaActual, $mesActual, $anioActual).
            MostrarObservaciones($conexion, $conjuntoDatos['USPR_ID'],3, $diaActual, $mesActual, $anioActual).
            MostrarObservaciones($conexion, $conjuntoDatos['USPR_ID'],4, $diaActual, $mesActual, $anioActual).
            MostrarObservaciones($conexion, $conjuntoDatos['USPR_ID'],5, $diaActual, $mesActual, $anioActual).
            MostrarObservaciones($conexion, $conjuntoDatos['USPR_ID'],6, $diaActual, $mesActual, $anioActual)
            ."</tr><tr>".
            MostrarTotalHorasExtra($conexion, $conjuntoDatos['USPR_ID'], $diaActual, $mesActual, $anioActual).
            "<td class='alturaDoble'>0</td>
            <td class='alturaDoble'>0</td>
            <td class='alturaDoble'>0</td>
            <td class='alturaDoble'>0</td>
            <td class='alturaDoble'>0</td>
    ...
    function MostrarTotal($conexion, $Id, $diaActual, $mesActual, $anioActual){
        $N_ASIS = 0;
        for($dia = 1; $dia <= $diaActual; $dia++){
            $fecha = $anioActual.'-'.$mesActual.'-'.$dia;
    ...
    function MostrarTotalHorasExtra($conexion, $Id, $diaActual, $mesActual, $anioActual){
        ...
        echo '<td class="alturaDoble">'.$N_HEXTRA.'</td>';
    }
    ...
    function MostrarObservaciones($conexion, $Id, $OBS_ID, $diaActual, $mesActual, $anioActual){
        ...
        for($dia = 1; $dia <= $diaActual; $dia++){
            $fecha = $anioActual.'-'.$mesActual.'-'.$dia;
            $Select_Asistencia = "SELECT count(*) FROM `tbl_asistencia` WHERE ((USPR_ID = '{$Id}')AND(OBS_ID = '{$OBS_ID}')AND(ASIS_FECHA = '$fecha'))";
            $Res_Asistencia = mysqli_query($conexion, $Select_Asistencia);
            $ARR_ASIS = mysqli_fetch_assoc($Res_Asistencia);
        ...
        echo '<td class="alturaDoble">'.$cont.'</td>';
    }
?>