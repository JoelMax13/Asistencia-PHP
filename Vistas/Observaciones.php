...
<?php
    ...
    function VerificarAsistencia($conexion, $UsPr_ID, $USU_NOMAPE){
        $Select_ConRegistro = "SELECT ASIS_ID, USPR_ID FROM tbl_asistencia 
        ...
        if($Registrado == NULL){
            ...
            $Resultado_AsisId = mysqli_query($conexion, $Select_AsisId);
            $ASIS_ID = mysqli_fetch_assoc($Resultado_AsisId);
            if($ASIS_ID != NULL){
                echo 
                '<li class="mb-3 row">
                    <label class="col-sm-4 col-form-label">'.$USU_NOMAPE.'</label>
                    <div class="col-sm-3"> 
                            <select class="form-select" name="sel'.$ASIS_ID['ASIS_ID'].'">';
